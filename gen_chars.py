...
def main():
    ...
    parser.add_argument("--font_char", type=str, default=None)
    ...
    if args.font_char:
        # Calculate font size to fit within bar dimensions
        # Use a "common" size as a base for scaling, then get the bounding box of the character
        # to calculate the proper scale.
        temp_font_size = 100
        temp_font = ImageFont.truetype(args.font_path, temp_font_size)
        bbox = temp_font.getbbox(args.font_char)
        # Bounding box is (x0, y0, x1, y1), so width is x1-x0, height is y1-y0
        char_width = bbox[2] - bbox[0]
        char_height = bbox[3] - bbox[1]
        
        if args.vertical:
            scale_factor = args.bar_width / char_height
            font_size = int(temp_font_size * scale_factor)
        else:
            scale_factor = args.bar_height / char_height
            font_size = int(temp_font_size * scale_factor)
            
        args.font = ImageFont.truetype(args.font_path, font_size)
